...
#import "YJSpecial.h"
#define YJStatusTextViewCoverTag 999
...
- (void)setupSpecialRects
{
    ...
    for (YJSpecial *special in specials) {
...
/**
 找出被触摸的特殊字符串
 */
- (YJSpecial *)touchingSpecialWithPoint:(CGPoint)point
{
    NSArray *specials = [self.attributedText attribute:@"specials" atIndex:0 effectiveRange:NULL];
    for (YJSpecial *special in specials) {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    // 根据触摸点获得被触摸的特殊字符串
    YJSpecial *special = [self touchingSpecialWithPoint:point];
    ...
    for (NSValue *rectValue in special.rects) {
        ...
        cover.tag = YJStatusTextViewCoverTag;
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (UIView *child in self.subviews) {
        if (child.tag == YJStatusTextViewCoverTag) [child removeFromSuperview];
    }
}
